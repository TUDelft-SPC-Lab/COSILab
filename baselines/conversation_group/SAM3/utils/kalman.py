...
def smooth_scale_shape_local(mhr, num_frames, window=9,
    ...
    scale_np = scale.detach().cpu().float().numpy().reshape(num_frames, num_humans, D_scale)
    shape_np = shape.detach().cpu().float().numpy().reshape(num_frames, num_humans, D_shape)
