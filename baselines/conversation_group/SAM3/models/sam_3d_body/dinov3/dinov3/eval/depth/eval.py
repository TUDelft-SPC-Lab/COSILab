...
def evaluate_depther_with_dataloader(
    ...
    final_metric_values_dict = dict(zip(metric_names, final_metric_values.cpu().float().numpy()))

    if reduce_results:
        out_results_dict = {k: float(v) for (k, v) in final_metric_values_dict.items()}
    else:
        out_results_dict = {
            metric_name: value for (metric_name, value) in zip(metric_names, all_metric_values.cpu().float().numpy().tolist())
        }
    logger.info(
