...
class SAM3DBody(BaseModel):
    ...
    def run_inference_batch(
        ...
        # Re-run forward
        with torch.no_grad():
            # If enabled, skip temporal smoothing/shape-freezing to expose "raw" per-frame outputs.
            # This is useful for a decoupled Stage2 (raw params dump) -> Stage3 (sequence smoothing) pipeline.
            disable_temporal_smoothing = os.environ.get("SAM3DBODY_DISABLE_TEMPORAL_SMOOTHING", "").strip() not in ("", "0", "false", "False")
            ...
            if not disable_temporal_smoothing:
                pose_output["mhr"] = kalman_smooth_mhr_params_per_obj_id_adaptive(
    ...
    def _get_hand_box(self, pose_output, batch):
        ...
        pred_left_hand_box = (
            ...
            pose_output["mhr"]["hand_box"][:, 0].detach().float().cpu().float().numpy()
            * self.cfg.MODEL.IMAGE_SIZE[0]
        )
        pred_right_hand_box = (
            # NumPy doesn't support BF16. Cast to FP32 before converting.
            pose_output["mhr"]["hand_box"][:, 1].detach().float().cpu().float().numpy()
            * self.cfg.MODEL.IMAGE_SIZE[0]
        )
        ...
        for bi in range(batch_size):
            batch["left_scale"][bi*num_objects:(bi+1)*num_objects] = (
                batch["left_scale"][bi*num_objects:(bi+1)*num_objects]
                / batch["affine_trans"][bi, :, 0, 0].cpu().float().numpy()[:, None]
            )
            ...
            batch["left_center"][bi*num_objects:(bi+1)*num_objects] = (
                batch["left_center"][bi*num_objects:(bi+1)*num_objects]
                - batch["affine_trans"][bi, :, [0, 1], [2, 2]].cpu().float().numpy()
            ) / batch["affine_trans"][bi, :, 0, 0].cpu().float().numpy()[:, None]
