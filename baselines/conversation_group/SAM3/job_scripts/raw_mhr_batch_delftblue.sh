...
# Batch Stage 2: raw MHR params (raw_mhr.pt) for each bbox_kp/<NUM> folder.
#
# Each folder (e.g. 128, 223, 228) must contain images/, masks/, id_mapping.json.
# Creates raw_mhr.pt under each folder. Uses a single intrinsics file for all tasks.
# Snapshots code to bbox_kp/_snapshots. Does NOT use sam4d or any bbox pkl paths.
#
# Usage (single folder):
#   sbatch job_scripts/raw_mhr_batch_delftblue.sh 228
#   sbatch job_scripts/raw_mhr_batch_delftblue.sh 128
#
# Usage (all folders with digit prefix):
#   sbatch job_scripts/raw_mhr_batch_delftblue.sh -all 2
#     -> processes all 2xx folders under bbox_kp (e.g. 223, 228, ...)
#   sbatch job_scripts/raw_mhr_batch_delftblue.sh -all 1
#     -> processes all 1xx folders (e.g. 128, ...)
#
# Paths (host):
...
#   Snapshots:     /scratch/zli33/data/conflab/bbox_kp/_snapshots
...
if [ -z "${INPUT_ARG:-}" ]; then
  ...
  echo "  Usage: sbatch job_scripts/raw_mhr_batch_delftblue.sh 228" >&2
  echo "         sbatch job_scripts/raw_mhr_batch_delftblue.sh -all 2" >&2
  exit 2
fi

# ---------------------------------------------------------------------------
# Build list of folder names (e.g. 128, 223) under bbox_kp
# ---------------------------------------------------------------------------
folder_nums=()
...
# ---------------------------------------------------------------------------
# Process each folder: run_sam3d_body_raw_params.py -> raw_mhr.pt
# (No sam4d or bbox pkl bindings in this pipeline.)
# ---------------------------------------------------------------------------
total=${#folder_nums[@]}
idx=0
failed=0
...
for FOLDER_NUM in "${folder_nums[@]}"; do
  ...
  input_container="$bind_data_path_conflab/bbox_kp/$FOLDER_NUM"

  if [ ! -d "$data_path_conflab/bbox_kp/$FOLDER_NUM" ]; then
    echo "[WARN] Folder does not exist on host: $data_path_conflab/bbox_kp/$FOLDER_NUM. Skipping."
    failed=$((failed + 1))
    echo ""
    continue
  fi
  ...
  echo "[INFO] output=$input_container/raw_mhr.pt"
  ...
  if apptainer exec --nv \
    ...
    python $project_folder/run_sam3d_body_raw_params.py \
      ...
      --out "$input_container/raw_mhr.pt" ; then
