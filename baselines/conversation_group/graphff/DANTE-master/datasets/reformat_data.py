...
def augment_frame_nonstandard(frame, n_features):
	...
	# if (len(people))%2==1:
	# 	bound = int(len(people)/n_features)+4
	# else:
	# 	bound = int(len(people)/n_features)
...
def write_shifted_frame(handle, frame):
	handle.write(' '.join(str(value) for value in frame) + '\n')
...
def shift_all_data_nonstandard(Positions, Groups_at_time, n_people, n_features, n_augmented_features, velocities, dataset):
	...
	print("time of groups",len(Groups_at_time))
	with open('../datasets/' + dataset + '/coordinates.txt', 'w') as f:
		for time in Groups_at_time:
			frame_idx = list(Positions[:,0]).index(time)
			frame = Positions[frame_idx]
			people = frame[1:]
			...
			# skip fake (placeholder people). Keep the original row order: all
			# normal directed pairs first, then all horizontally flipped pairs.
			for i in range(n_people):
				...
				for j in range(n_people):
					...
					write_shifted_frame(f, ij_frame)
			...
			for i in range(n_people):
				...
				for j in range(n_people):
					...
					write_shifted_frame(f, flipped_frame)
...
def write_affinities_and_timechanges_from_coordinates(dataset, Groups_at_time):
	coordinates_path = '../datasets/' + dataset + '/coordinates.txt'
	affinities_path = '../datasets/' + dataset + '/affinities.txt'
	timechanges_path = '../datasets/' + dataset + '/timechanges.txt'

	i = 0
	prev_time_arr = [-1, -1]
	timechanges = []
	with open(coordinates_path, 'r') as coordinates_handle, \
			open(affinities_path, 'w') as affinities_handle:
		for line in coordinates_handle:
			line = line.strip()
			if not line:
				continue
			time = line.split(' ', 1)[0]
			...
			if prev_time_arr[0] != time_arr[0] or prev_time_arr[1] != time_arr[1]:
				timechanges.append(i)
			affinity = indiv_affinities_at_time(time, Groups_at_time)
			affinities_handle.write(str(time) + ' ' + str(affinity) + '\n')
			i += 1
			prev_time_arr = time_arr

	np.savetxt(timechanges_path, timechanges, fmt='%s')
...
if __name__ == "__main__":
	...
	print("data shifted")
	write_affinities_and_timechanges_from_coordinates(dataset, Groups_at_time)
