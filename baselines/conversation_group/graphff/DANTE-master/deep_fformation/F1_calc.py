...
def F1_calc(group_thres, affinities, times, Groups_at_time, Positions, n_people, ds_iteration_thres, n_features, non_reusable=False, dominant_sets=True):
	...
	while not done:
		...
		if dominant_sets:
			bool_groups,original_A = iterate_climb_learned(predictions, frame, n_people, thres=ds_iteration_thres, n_features=n_features)
		else:
			bool_groups = naive_group(predictions, frame, n_people, thres=ds_iteration_thres, n_features=n_features)

		original_affinities.append(original_A) # original_A should be np array
	...
	return f1_avg, avg_results[0], avg_results[1],original_affinities, frames
