...
# Submit exactly one intention-inference job WITHOUT personas.
#
# There is one entry point and one scale: submit this file with sbatch and give
# the inclusive manifest range that should fit in this job. For example:
#
#   sbatch job_scripts/intention/cosilab_daic.sh \
#       --backend gemma --mode pa --index-range 0-999
#
# It deliberately does not tile or submit itself. Plain runs are large enough
# that submitting their few ranges explicitly is clearer than carrying a second
# launcher interface. For persona runs, including automatic multi-job packing,
# use persona.sh.
#
...
# Absolute host paths. These are NOT container paths: everything here runs on the
# host, before apptainer is involved. PROJECT_ROOT is what gets bound to
...
# Outside Slurm only --help is supported. Discovering the checkout there keeps
# help usable from any clone. Under sbatch, BASH_SOURCE points at Slurm's spooled
# copy, so the hardcoded cluster checkout remains the source of truth.
...
# persona.sh submits this same worker with an internal marker so the expensive
# container and inference path remains single-sourced. Users should not submit
# persona flags here directly; persona.sh owns source validation, packing and
# resume checks.
persona_job="${INTENTION_PERSONA_JOB:-0}"
if [[ "${persona_job}" != "0" && "${persona_job}" != "1" ]]; then
    echo "[ERROR] INTENTION_PERSONA_JOB must be 0 or 1, got: ${persona_job}" >&2
    exit 1
fi
...
usage() {
    ...
    echo "" >&2
    echo "Persona runs: bash job_scripts/intention/persona.sh --help" >&2
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --project-root)
            ...
            echo "        environment, so set PROJECT_ROOT near the top of this script." >&2
            exit 1
            ;;
        -h|--help)
            usage
...
# ---------------------------------------------------------------------------
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    echo "[ERROR] cosilab_daic.sh is a Slurm job, not a launcher." >&2
    echo "        Submit one no-persona range with sbatch, for example:" >&2
    echo "          sbatch job_scripts/intention/cosilab_daic.sh \\" >&2
    echo "              --backend gemma --mode pa --index-range 0-999" >&2
    echo "        For automatic persona jobs, run persona.sh with bash." >&2
    exit 1
fi

if [[ "${persona_job}" == "0" && "${persona_option_seen}" == "1" ]]; then
    echo "[ERROR] Persona options are submitted through persona.sh, not directly through" >&2
    echo "        cosilab_daic.sh. The launcher validates the source, sizes the jobs and" >&2
    echo "        prevents two jobs from writing the same persona output." >&2
    exit 1
fi
if [[ "${persona_job}" == "1" && -z "${assignment_json}" && -z "${task_spec}" ]]; then
    echo "[ERROR] Internal persona job has neither --assignment-json nor --task-spec." >&2
    echo "        Submit persona work through job_scripts/intention/persona.sh." >&2
    exit 1
fi
...
if [[ ! -f "${SRC_DIR}/intention_inference/cli.py" ]]; then
    ...
    echo "        Set PROJECT_ROOT near the top of this script to your COSILab checkout." >&2
    exit 1
fi
