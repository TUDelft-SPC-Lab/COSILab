...
from capturesystem.video_utils import (
...
def timedelta_to_hhmmssff(td: timedelta, framerate: float) -> str:
    """Convert a timedelta to HHMMSSFF format (Hours:Minutes:Seconds:Frames)."""
    total_seconds = td.total_seconds()
    hours = int(total_seconds // 3600)
    minutes = int((total_seconds % 3600) // 60)
    seconds = int(total_seconds % 60)
    fractional_seconds = total_seconds - int(total_seconds)
    frames = int(fractional_seconds * framerate)
...
def extract_segment_from_video(
    ...
    for source_camera_video_path in tqdm(
        ...
        # Get framerate early to format the output filename
        ...
        # Build output filename with start/end times
        base_name = source_camera_video_path.stem
        suffix = source_camera_video_path.suffix
        ...
        if start_time is not None:
            start_str = f"_{timedelta_to_hhmmssff(start_time, framerate)}"
        
        if end_time is not None:
            end_str = f"_{timedelta_to_hhmmssff(end_time, framerate)}"
        
        output_filename = f"{base_name}{start_str}{end_str}{suffix}"
        camera_target_path = target_directory / output_filename
        camera_target_path.parent.mkdir(exist_ok=True)
        ...
        timecode = VideoTimecode.from_video(source_camera_video_path)
        new_timecode = timecode.to_ffmpeg_format()
        ...
        if use_timecode is True:
            if start_time is None and end_time is None:
                ...
                timedelta = timecode.to_timedelta()
                if start_time is not None:
                    start_time_i = start_time - timedelta
                if end_time is not None:
                    end_time_i = end_time - timedelta
        else:
        ...
        if start_time_i is not None:
            ...
            new_timecode = VideoTimecode.from_timedelta(
                timecode.to_timedelta() + start_time_i, timecode.fps
            )

            new_timecode = new_timecode.to_ffmpeg_format()
        ...
        ffmpeg = ffmpeg.output(
            str(camera_target_path),
            **output_options,
            map="0",  # use all streams from input
            c="copy",  # copy codec to avoid re-encoding
            dn=None,  # disable data streams, avoid errors with tmcd streams
